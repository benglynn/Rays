...
@IBDesignable class RaysView: UIView {
    ...
    private let startBackgroundColour = UIColor(red: 250/255, green: 0/255, blue: 41/255, alpha: 1.0).cgColor
    private let endBackgroundColour = UIColor.white.cgColor
    ...
    func endTransform(for rect: CGRect) -> CATransform3D {
        ...
        return restingTranslateRotate
    }
    
    func drawRayLayers(in rect: CGRect) -> [CALayer] {
        let raysCenter = CGPoint(x: rect.midX, y: rect.midY)
        let rayLength = 2 * CGFloat(max(rect.height, rect.width))
        return rayShapes.enumerated().map { (index, ray) -> CAShapeLayer in
    ...
    func addUnderlyingAnimation(to layer: CALayer, for rect: CGRect) {
        CATransaction.setDisableActions(true)
        layer.backgroundColor = startBackgroundColour
        ...
        anim.toValue = endTransform(for: rect)
        anim.fillMode = kCAFillModeBackwards
        layer.add(anim, forKey: nil)
    }
    
    func addRayAnimation(at index: Int, to rayLayer: CALayer) {
    ...
    func isAnimating() -> Bool {
        return isFirstViewOfSession && !isInterfaceBuilder
    }
    
    override func draw(_ rect: CGRect) {
        removeLayers()
        layer.transform = endTransform(for: rect)
        layer.backgroundColor = endBackgroundColour
        let rayLayers = drawRayLayers(in: rect)
        
        if isAnimating() {
            addUnderlyingAnimation(to: layer, for: rect)
            rayLayers.enumerated().forEach() { (index, rayLayer) in addRayAnimation(at: index, to: rayLayer) }
        }
